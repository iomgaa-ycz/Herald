...
2. **计划 (Plan)**
   ...
   - **1.3 流程图/伪代码** — 需要以高密度信息密度的形式以伪代码或者流程图的形式给出对于本次修改以及与现有代码嵌合的解释，方便人类评估修改是否可行。
   - **1.3 拟议变更 (Proposed Changes)** — 精确到函数级别，标识 `[NEW]`/`[MODIFY]`/`[DELETE]`
   - **1.4 验证计划 (Verification Plan)** — 怎么确认做对了

   **计划一致性约束**
   - 计划先定义唯一真值主链路（事件流/调用流/数据流），后续章节必须严格对齐，不得并存第二套语义。
   - 架构图、组件职责、核心接口必须同构；若某处新增角色/事件/职责，必须同步改其余三处。
   - 禁止复用旧接口名承载新语义；若旧名已绑定语义，新增入口必须改名。
   - 文件变更清单必须覆盖启动接线点；验证方案必须覆盖主链路闭环，而非仅验证局部模块。
...
- **功能修改**: 不考虑向后兼容，代码简洁优先
- **任务边界**: **严禁添加任务范畴之外的代码**。只做计划中明确要求的事，不自行补充示例、模板、配置等内容。后续任务会逐步完善。
