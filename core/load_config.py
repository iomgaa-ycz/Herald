...
class ConfigManager:
    ...
    def _resolve_actual_type(self, cls: Any, field_name: str) -> Any:
    ...
    def _deep_update(self, base: dict, updater: dict):
        """递归合并字典。"""
    ...
    def _smart_cast(self, type_hint):
        """处理 argparse 的类型转换逻辑。"""
        if type_hint == bool:
    ...
    def parse(self) -> HeraldConfig:
        ...
        for field_name, field_type in all_cli_fields.items():
            ...
            cli_parser.add_argument(f"--{cli_arg_name}", type=self._smart_cast(field_type))
        ...
        for field in fields(cls):
            if field.name in data:
                ...
                if is_dataclass(actual_type) and isinstance(field_val, dict):
                    init_args[field.name] = self._dict_to_dataclass(actual_type, field_val)
                else:
